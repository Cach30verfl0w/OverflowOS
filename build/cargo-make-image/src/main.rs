...
fn main() {
    ...
    info!("     {}, written in {} with {}", "Operating System".gradient(Color::Blue), "Rust".color(Color::Orange3), "UEFI".red());
    ...
    let cargo_path = match find_in_path("cargo") {
        Some(path) => path,
        None => {
            error!("Unable to recognize path of Cargo {} {} Cargo executable found in $PATH", "=>".color(Color::DarkGray), "No".red());
            exit(-1);
        }
    };
    ...
    if !crates_directory.exists() {
        error!("Unable to build project => {} to find crates directory! Aborting execution...", "Unable".red());
        exit(-2);
    }
