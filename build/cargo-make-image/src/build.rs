...
pub(crate) fn build_projects_with_cargo(
    cargo_path: PathBuf, crates_directory: &Path, args: &Arguments,
) -> Result<(), Error> {
    ...
    let image_generator =
        ImageGenerator::new(image_path.join(&args.image_file), args.sector_size, args.sector_count)?;
    ...
    for entry in read_dir(crates_directory)? {
        ...
        info!(
            "Building cargo project {} with target {}",
            package.name.clone().gradient(Color::Green),
        ...
        if args.bootloader.eq(package.name()) {
            if args.architecture.is64bit() {
                image_generator.copy_into(
                    format!("target/{}/debug/{}.efi", target, package.name()),
            ...
            image_generator.copy_into(
                format!("target/{}/debug/{}", target.replace(".json", ""), package.name()),
                format!("{}.ELF", package.name()).to_uppercase(),
            )?;
        }
    }
